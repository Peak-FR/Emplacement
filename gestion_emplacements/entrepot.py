...
class Entrepot:
    def __init__(self):
        """
        L'entrepôt n'a plus besoin de stocker les allées en mémoire ici,
        car il opérera directement sur la base de données via des sessions.
        """
        pass
    ...
    def initialiser_entrepot_en_base_de_donnees(self, session: Session):
        ...
        if session.query(AlleeDB).filter_by(lettre_allee=premiere_allee_attendue).first() and \
           session.query(RackDB).filter_by(id_rack_complet_str=f"{premiere_allee_attendue}1").first():
            print("L'entrepôt semble déjà initialisé en base de données. Initialisation ignorée.")
            return
        print("Initialisation de la structure de l'entrepôt en base de données...")
        ...
        try:
            ...
            session.commit()
            print("Entrepôt initialisé et sauvegardé en base de données.")
        except Exception as e:
            session.rollback()
            print(f"Erreur lors de l'initialisation de l'entrepôt en BDD : {e}")
            raise


    def get_emplacement_par_id_complet(self, session: Session, id_emplacement_str: str) -> EmplacementDB | None:
        """
        Récupère un objet EmplacementDB spécifique à partir de son ID complet 
    ...
    def trouver_emplacement_libre_optimal(self, session: Session, taille_recherchee: str, 
        ...
        try:
            ...
            if chercher_uniquement_allee_J:
                print("INFO: Recherche uniquement en Allée J (avec priorité niveaux hauts).")
                ...
                print("INFO: Recherche en mode éparpillement total (avec priorité niveaux hauts).")
                if exclure_allee_J_par_defaut:
                    query = query.filter(EmplacementDB.allee_lettre != 'J')
                # D'abord les niveaux 1-7 au hasard, puis les niveaux 8-10 au hasard
                query = query.order_by(case_priorite_niveau, func.random())
            else:
                print("INFO: Recherche avec priorisation standard (avec priorité niveaux hauts).")
                if exclure_allee_J_par_defaut:
                    query = query.filter(EmplacementDB.allee_lettre != 'J')
                ...
                query = query.order_by(
                    case_priorite_niveau,               # 1. Groupe de priorité de niveau (0 pour 1-7, 1 pour 8-10)
                    EmplacementDB.niveau,               # 2. Numéro de niveau exact (1, 2, ...)
                    EmplacementDB.rack_numero,          # 3. Numéro de Rack (1, 2, ...)
                    case_ordre_allees,                   # 4. Allée (A, B, ...)
                    EmplacementDB.position_dans_niveau, # 5. Position
                    func.random()                       # 6. Aléatoire
                )
            ...
            if emplacement_optimal:
                if chercher_uniquement_allee_J:
                    print(f"INFO: Emplacement trouvé (Allée J): {emplacement_optimal.id_emplacement_str}")
                elif mode_eparpillage_total:
                     print(f"INFO: Emplacement trouvé (Éparpillement Total): {emplacement_optimal.id_emplacement_str}")
    ...
    def assigner_produit_a_emplacement(self, session: Session, id_emplacement_str: str, 
        ...
        """
        Tente d'assigner un produit à un emplacement. ID Produit est maintenant obligatoire.
        NE FAIT PAS DE COMMIT.
        Retourne (succès_bool, message_str, emplacement_modifié_ou_None).
        """
        if not id_produit: 
            return False, "L'ID Produit est manquant et obligatoire.", None
        
        # 1. Vérifier si ce produit_id spécifique existe déjà ailleurs
        ...
        if emplacement_existant_du_produit:
            if emplacement_existant_du_produit.id_emplacement_str == id_emplacement_str:
                return False, f"Produit (ID: {id_produit}) est déjà présent dans cet emplacement '{id_emplacement_str}'.", emplacement_existant_du_produit
            else:
        ...
        # 2. Récupérer l'emplacement cible
        ...
        # 3. Vérifier si l'emplacement cible est libre
        ...
        # 4. Si toutes les vérifications sont passées, on peut assigner
        ...
        return True, f"Produit '{nom_produit}' (ID: {id_produit}) prêt à être assigné à '{id_emplacement_str}'.", emplacement
        ...
        try:
            ...
            if not succes_marquage:
                return {
                    ...
                    "message_operation": f"Échec du marquage comme libre pour {id_emplacement_str}, état inattendu.",
            ...
            return {
                ...
                "message_operation": f"Prêt pour libération.",
                "ancien_produit_nom": nom_produit_avant,
                "ancien_produit_id": id_produit_avant
            }
        ...
        if ids_necessitant_commit:
            try:
                ...
                for id_emp in ids_necessitant_commit:
                    resultats_par_id[id_emp]["status_final"] = "Libéré avec succès"
                ...
                session.rollback()
                print(f"Erreur lors du commit de la libération en masse : {e}. Rollback effectué.")
                for id_emp in ids_necessitant_commit:
                    resultats_par_id[id_emp]["status_final"] = "Échec (Rollback)"
                    resultats_par_id[id_emp]["message_final"] = "Libération annulée (erreur de sauvegarde globale)."
        else:
            print("Libération en masse : Aucun emplacement n'a nécessité de modification pour libération.")
    ...
    def assigner_produits_en_masse(self, session: Session, produits_a_assigner: list, 
        ...
        for prod_data in produits_a_assigner:
            ...
            if not nom_produit or not taille_requise or not id_produit:
                resultats_batch.append({
                    ...
                    "statut": "Échec", "message": "Données produit incomplètes (nom, ID produit et taille requis)."
                })
                continue
            ...
            # Trouver un emplacement optimal en excluant ceux déjà pris pour ce lot
            ...
            if not emplacement_optimal:
                resultats_batch.append({
                    "produit_nom": nom_produit, "id_produit": id_produit,
                    "statut": "Échec", 
                    "message": f"Aucun emplacement libre de taille '{taille_requise}' trouvé (en tenant compte des réservations du lot)."
                })
                continue
            ...
            if success_assign:
                ...
                resultats_batch.append({
                    "produit_nom": nom_produit, "id_produit": id_produit,
                    "statut": "Échec", 
                    "emplacement_tenté": emplacement_optimal.id_emplacement_str,
                    "message": msg_assign
                })
        ...
        if any(r["statut"] == "Assigné" for r in resultats_batch):
            try:
                ...
                print(f"Assignation en masse : {nb_assignes} produits assignés et commités (Mode éparpillement: {mode_eparpillage_total_global}).")
            except Exception as e:
                session.rollback()
                ...
                for res in resultats_batch:
                    if res["statut"] == "Assigné":
                        res["statut"] = "Échec (Rollback)"
                        res["message"] = "L'assignation a été annulée (erreur de sauvegarde globale)."
        else:
            print("Assignation en masse : Aucun produit n'a pu être assigné dans ce lot.")
        
        return resultats_batch

    def get_allee_details_complete(self, session: Session, lettre_allee_cible: str) -> dict | None:
        """
        Récupère les détails complets d'une allée, y compris tous ses racks et leurs emplacements.
        """
    ...
    def verifier_si_produit_existe(self, session: Session, id_produit: str) -> EmplacementDB | None:
        """
        Vérifie si un produit avec cet id_produit est déjà assigné à un emplacement.
        Retourne l'objet EmplacementDB si trouvé, sinon None.
        """
        if not id_produit:
            return None
    ...
    def rechercher_produit(self, session: Session, terme_recherche: str) -> list[EmplacementDB]:
        """
        Recherche des produits par ID exact ou par nom partiel (insensible à la casse).
        Retourne une liste d'objets EmplacementDB où les produits sont trouvés.
        """
    ...
    def get_statistiques_entrepot(self, session: Session) -> dict:
        """
        Calcule et retourne des statistiques de base sur l'occupation de l'entrepôt.
        """
        stats = {}
    ...
    def get_historique_mouvements(self, session: Session, 
        ...
        """
        Récupère l'historique des mouvements avec pagination et filtres optionnels.
        """
        if page < 1:
            page = 1
